...
```{r}
train_iter<- mx.io.arrayiter(data = t(train_x), label = train_y, batch.size = 16, shuffle = TRUE)
eval_iter<- mx.io.arrayiter(data = t(eval_x), label = eval_y, batch.size = 16, shuffle = FALSE)
```
...
Afin que le modèle puisse apprendre, des valeurs aléatoires sont assignées aux paramètres du modèle avant l'exécution de l'optimisation. Différentes approches sont disponibles:  
...
init_weight<- mx.init.create(initializer_uniform, shape.array = shapes$arg.shapes, ctx = mx.cpu())
```

Poids initiaux de la dernière couche: 

```{r, echo=FALSE}
as.array(init_weight$final_weight)
```
